...
def train_rl_agent(viz):
    ...
    optimizer = optim.Adam(q_net.parameters(), lr=1e-4)  # 降到 1e-4 更穩定
    replay_buffer = ReplayBuffer(30000)

    score_history = []
    steps_done = 0

    # === 關鍵1：Warm-up 先隨機填 15000 筆經驗（避免初期壞經驗污染）===
    ...
    for _ in range(130000):
        ...
        if _ % 100 == 0:  # 每 100 步強制 reset，避免單 episode 太長
            world.reset()
            warm_state = encode_state(world, [])
    ...
    for ep in range(RL_EPISODES):
        ...
        for step in range(RL_STEPS_PER_EP):
            ...
            # === 最終獎勵：只用原版 reward（-0.1 move, -1 wall, +10 coin），絕不加任何手刻 penalty ===
            base_reward, _ = world.step(action)
            reward = base_reward  # 就是這行！什麼都別加！
            ...
            if steps_done % 1000 == 0:  # 稍微頻繁一點更新 target
                target_net.load_state_dict(q_net.state_dict())
