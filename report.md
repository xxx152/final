...
- **架構**：
  ...
  ```
- **訓練**：
  - 目標：最大化累積獎勵（金幣 +10，碰撞 -1，移動 -0.1）。
  - 探索策略：ε-greedy，初始 ε=1.0，線性衰減至 0.1（避免過早收斂到局部最優）。
  - Replay Buffer：300,000 容量，減少早期經驗被覆蓋。
  - Target Network 更新頻率：每 3000 步，提升穩定性。
  - **結果**：3000 個回合訓練後，平均分數 ~15（5 個金幣，每個 +10）。
...
- **模型**：
  ...
  ```
- **訓練**：
  - 資料：由 RL 專家生成 150,000 個狀態-動作對。
  - 目標：交叉熵損失（分類），訓練 40 個 epoch。
  - 優化器：Adam(lr=5e-5)。
  - **結果**：top-2 預測準確率 ~85%。
...
**互動測試**（play_game.py）：
- 玩家按箭頭鍵控制，LSTM 預測玩家下一步。
- 若預測正確（Match=True），Latency 顯示 0ms（感知零延遲）。
- 若預測錯誤，延遲顯示為實際 RTT（300 幀 ÷ 60 FPS ÷ 2 = 2.5 秒，演示級別）。
- UI 實時顯示：Acc%, FPS, Top2 預測, 分數。
